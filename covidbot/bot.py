    ...
    def get_district_report(self, district_id: int) -> str:
        ...
        if current_data.icu_data:
            message += f"<b>🏥️ Intensivbetten</b>\n" \
                       ...
                       f" mit COVID-19, davon müssen {format_noun(current_data.icu_data.covid_ventilated, FormattableNoun.PERSONS)} invasiv beatmet werden. " \
    ...
    def _get_report(self, subscriptions: List[int]) -> str:
        ...
        if country.icu_data:
            message += f"<b>🏥 Intensivbetten</b>\n" \
                       ...
                       f" mit COVID-19, davon müssen {format_noun(country.icu_data.covid_ventilated, FormattableNoun.PERSONS)} ({format_float(country.icu_data.percent_ventilatedimport csv
import datetime
import logging
import re
import os.path
    ...
    def _get_report(self, subscriptions: List[int]) -> str:
        ...
        if country.icu_data:
            message += f"<b>🏥 Intensivbetten</b>\n" \
                       ...
                       f"In {format_noun(country.icu_data.occupied_covid, FormattableNoun.BEDS)} " \
                       f"({format_float(country.icu_data.percent_covid())}%) liegen Patient:innen" \
                       f" mit COVID-19, davon müssen {format_noun(country.icu_data.covid_ventilated, FormattableNoun.PERSONS)} invasiv beatmet werden. " \
